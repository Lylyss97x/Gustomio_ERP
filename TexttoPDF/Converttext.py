"""
GUSTOMIO — Commande catalogue → JSON → Odoo CRM
"""
...
# Charge le .env AVANT d'importer odoo_connector
load_dotenv()

# Chemin vers le dossier pipeline (dossier frère de TexttoPDF)
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'pipeline'))
import odoo_connector as odoo

# ══════════════════════════════════════════════
#  CATALOGUE GUSTOMIO 2026
# ══════════════════════════════════════════════

CATALOGUE = {
    "01 - CRÉMERIE / BEURRE": [
        {"code": "61362",    "libelle": "Beurre doux 250 g",                         "uv": "Pièce"},
    ...
    "04 - CHARCUTERIE / Jambon & Saucisson": [
        {"code": "62018",    "libelle": "Demi jambon cuit supérieur Le Mistral ~3,5kg","uv": "Kilo"},
...
def saisie_commande() -> dict:
    ...
    livraison = demander(
        ...
        msg_erreur="Format invalide ou date passée. Ex : 25/05/2026"
    )
    print("\n  Canal de réception :")
...
def envoyer_vers_odoo(commande: dict):
    print("\n" + "="*60)
    print("  ENVOI VERS ODOO")
    print("="*60)

    # Convertir JJ/MM/AAAA → AAAA-MM-JJ pour Odoo
    try:
        date_odoo = datetime.strptime(
            commande["client"]["date_livraison"], "%d/%m/%Y"
        ).strftime("%Y-%m-%d")
    except Exception:
        date_odoo = commande["client"]["date_livraison"]

    # Construire le JSON au format attendu par odoo_connector
    order_json = {
        "source":           commande["metadata"]["canal"],
        "client_name":      commande["client"]["nom"],
        "delivery_address": commande["client"]["adresse_livraison"],
        "delivery_date":    date_odoo,
        "items": [
            {
                "article_code":    art["code_article"],
                "label_source":    art["libelle"],
                "qty":             art["quantite"],
                "unit":            art["unite"],
                "confidence_item": 1.0,
            }
        ...
        "flags":            ["PRODUIT_NON_TROUVE_ODOO"],
        "recommendation":   "REVIEW",
    }

    try:
        result = odoo.route_order(order_json)
        icone  = "" if result["type"] == "sale_order" else "🔍"
        label  = "Sale Order" if result["type"] == "sale_order" else "CRM Lead"
        print(f"\n  {icone}  [{label}] créé dans Odoo : {result['ref']}")
        print(f"    https://h3-hitema.odoo.com")
    except Exception as e:
        print(f"\n    Erreur Odoo : {e}")
        print("      → JSON sauvegardé localement, envoi manuel nécessaire.")
...
def main():
    ...
    print(f"\n   Fichier JSON créé : {output}")

    # Envoi automatique vers Odoo
    envoyer_vers_odoo(commande)
